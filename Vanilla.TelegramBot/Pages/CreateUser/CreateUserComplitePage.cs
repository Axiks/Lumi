...
{
    public class CreateUserComplitePage(TelegramBotClient _botClient, UserContextModel _userContext, List<int> _sendMessages, IUserService _userService, UserActionContextModel _updateUserActionContextModel) : IPage, IPageKeyboardExtension
    {
        ...
        async void SaveUserAction(Update update)
        {
            ...
            try
            {
                var newUserModel = await _userService.RegisterUser(new UserRegisterModel
                {
                    TelegramId = _userContext.UpdateUser.TgId,
                    Username = _userContext.UpdateUser.Username,
                    Nickname = _updateUserActionContextModel.Nickname,
                    FirstName = _userContext.UpdateUser.Firstname,
                    LastName = _userContext.UpdateUser.Lastname,
                    About = _updateUserActionContextModel.About,
                    Links = _updateUserActionContextModel.Links,
                    LanguageCode = _userContext.UpdateUser.LanguageCode,
                    IsRadyForOrders = _updateUserActionContextModel.IsRadyForOrders,
                });

                var updateUserModel = await _userService.UpdateUserAsync(_userContext.UpdateUser.TgId, new UserUpdateRequestModel
                {
                    Images = _updateUserActionContextModel.Images,
                    IsHasProfile = _userContext.Roles.Contains(RoleEnum.User)
                });

                _userContext.User = updateUserModel; // update user model after save
            }
            catch (Exception ex) {
                _botClient.SendMessage(_userContext.UpdateUser.TgId, "An error occurred while creating your profile.");
            }
        }
